...
class Partner extends Base{
	...
	public function stockList() {
		...
		$this->assign('storeage', tpCache('basic.partner_storeage')); //配货员库存预警比例配置
			...
			if ($data['act'] == 'cancel') { //拒绝申请
				...
				if (!empty($delivery) && ($delivery['status'] == 1)) {
					...
					foreach ($goods as $key => $value) {
						...
						DB::name('partner_stock')->where(array('partner_id' => $this->partner_id, 'goods_id' => $value['goods_id']))->dec('goods_num', $value['goods_num'])->data('edittime', time())->update(); //直接扣除配货员商品库存数量
						$log = array(
		...
		if (IS_POST) {
			...
			if ($delivery_id) {
				foreach ($data['goods'] as $key => $value) {
					...
					DB::name('partner_stock')->where(array('partner_id'=>$this->partner_id, 'goods_id'=>$value['goods_id']))->dec('goods_num', $value['number'])->data('edittime', time())->update(); //直接扣除配货员商品库存数量
					$log = array(
						'partner_id' => $this->user_id,
						'goods_id' => $value['goods_id'],
						'delivery_id' => $delivery_id,
