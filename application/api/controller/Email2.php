...
class Email2 extends Controller{
...
//        $user_id = $user_id;//老板的ID
        ...
        //接收生成二维码的数量如果超过1000  再次调用
        $number = $params['number'];

        $again_thousand = $params['again_thousand'];//接收到的第N次调用 最后一次调用使用$number为余数 (有可能为空)
        $remainder = $params['remainder'];//余数 (有可能为空)
        if($again_thousand === 0){//最后一次调用
            $number = $params['remainder'];
//            $number = $params['number'];


        }elseif ($again_thousand >= 1){//再次执行1000
            $numebr = 1000;
        }
        elseif ($number>1000 && is_null($again_thousand)){//>1000的第一次调用
            $remainder = $number % 1000;//300  余数
            $number = 1000;
            $again_thousand  = floor($number / 1000);//例如7300  / 1000  再运行6次1000 一次300  (7)
...
//            $start = DB::name('client_luck_key')->where(['client_id' => $client_id])->max('key_id');
        ...
        for ($i = $start; $i < $number + $start; $i++) {
    		$this->pin($phone,$i,$again_thousand);//生成的编号和所属人电话
//            $this->pin($email, $i);//生成的编号和所属人邮箱
        ...
        $datalist = "./public/upload/qr_code/" . date("Y-m-d ") . "-" . $phone . "-" . $again_thousand . "/";
        $filename = "./public/upload/qr_code/" . date("Y-m-d ") . "-" . $phone . "-" . $again_thousand .".zip";
...
//        $this->send_email('qukaliujun@163.com','测试邮件','抽奖二维码',$filename);//发送压缩包
        ...
        if($email_result['error'] == 0){
            ...
            if ($again_thousand > 1){//证明>2000
                $data = array(
                    'number' => 1000,
                    'again_thousand' => $again_thousand - 1,
                    'out_trade_no' => $out_trade_no,

                );
                $url = "http://www.goldenbrother.cn/index.php/api/email/piliang";
                json_curl($url,$data);
...
//                $url = "http://www.goldenbrother.cn/index.php/api/email/piliang";
                $url = "http://192.168.1.144/api/email2/piliang";
                $end = json_curl($url,$data);
                halt($end);
            }
...
//
    ...
    /*拼接二维码*/
    public function pin($phone, $i, $again_thousand)
    {
        ...
        //文件夹的存放地址
        $qr_code_path = $_SERVER['DOCUMENT_ROOT'] . '/public/upload/qr_code/' . date("Y-m-d ") ."-". $phone . "-" . $again_thousand .  '/';
...
//    delFile($filename);
    ...
    public function font(){
        echo "THINK_VERSION:".THINK_VERSION;
    }
