...
class Partner extends Base {
		...
		if (IS_POST) {
			...
			if (empty($partner_id)) {
				...
				if (!$user_id) {
					...
					$partner_id = DB::name('partner')->add($partner); //插入配送员表
					if (!$partner_id) {
						$this->ajaxReturn(['status' => 0, 'msg' => '添加失败','result' => '']);
					}else {
						adminLog('添加配送员 ' . $user['nickname']); //记录日志
				...
				$partner['town_id'] = $user['district'];
				$partner['phone'] = $user['mobile'];
				$res2 = DB::name('partner')->where(array('partner_id' => $partner_id))->save($partner); //更新配送员表

				if (($res1 === false) || ($res2 === false)) {
					$this->ajaxReturn(['status' => 0, 'msg' => '编辑失败','result' => '']);
				} else {
					adminLog('编辑配送员 ' . $user['nickname']); //记录日志
	...
	public function editWithdrawals() {
		...
		if (IS_POST) {
			...
			if ($r) {
				if ($data['status'] == 1 && $record['status'] != 1) {
					adminLog('管理员通过了配送员-' . $record['nickname'] . '的提现申请');
				}else if ($data['status'] == 2) {
					adminLog('管理员拒绝了配送员-' . $record['nickname'] . '的提现申请');
				}
	...
	public function delWithdrawals() {
		...
		adminLog('管理员删除了配送员-' . $nickname . '的提现申请');
	...
	public function ajaxStockList() {
		...
		$this->assign('storeage', tpCache('basic.partner_storeage')); //配送员库存预警比例配置
		return $this->fetch();
	}
	...
	public function delivery() {
		...
		if (IS_POST) {
			...
			$partner['delivery_type'] = 2; //公司发给配送员
			...
			if ($id) {
				...
				adminLog('管理员对配送员-' . $partner['name'] . '进行补货操作！');
				$this->ajaxReturn(array('status'=>1, 'msg'=>'操作成功！'));
			}else {
				$this->ajaxReturn(array('status'=>0, 'msg'=>'操作失败！'));
			}
