    ...
    public function delivery()
    {
    	...
    	if (IS_POST) {
    		if ($machine_id) {
    			...
    			if ($r !== NULL) {
    				$stock_arr = DB::name('machine_conf')->where(['machine_id'=>$mahcine_id])->getField('goods_id',true);
    				$stock =implode(',',$stock_arr);//此机器的商品ids
    		...
    		if($stock){
	    		...
	    		//修改配货员此商品的最大库存 machine_stock中goods_num不为0不会被删除
	    	
	    		$partner_stock = DB::name('partner_stock')->where("goods_id in ({$stock})")->select();
	    		$partner_arr = DB::name('partner_stock')->where(['partner_id'=>$partner_id])->gitField('goods_id',true);
	    		$partner_ids = implode(',',$partner_arr);
	    		$goods_arr = M('machine_conf')->where('machine_id',$machine_id)->getField('goods_id',true);//新增的数据
	    		$goods_ids = implode(',',$goods_arr);//ids重复不改 不重复partner_stock(删/增),(删,加),(减/加),(减,增)同 时进行  
	    		$new_ids = array_diff_assoc($goods_arr,$partner_arr);//新增的ids
	    			foreach ($machine_conf as $k => $v) {// $mahcine_conf新增的数据  找出不同的
	 					foreach($new_ids as $key => $value){
		    				if($v['goods_id']==$value){
		    					//经过修改的商品
		    					if(in_array($v['goods_id'], $partner_arr)){
		    						//存在 增加
		    						DB::name('partner_stock')->where(['goods_id'=>$v['goods_id']])->setInc('stock_num',$v['goods_num']);
		    						//增加<-->减少(删除)
		    					
		    						// $default_arr = array_merge(array_diff($partner_arr,$goods_arr),array_diff($goods_arr,$partner_arr));//发生改变的goods_id  原始排在前 新更排在后
		    						...
		    						$res = DB::name('partner_stock')->where(['goods_id'=>$del_goods_value])->getField('stock_num');
		    						if ($res - $v['goods_num'] <= 0 ) {
		    							DB::name('partner_stock')->where(['goods_id'=>$del_goods_value])->delete();
		    						}else{
		    							DB::name('partner_stock')->where(['goods_id'=>$del_goods_value])->setDec('stock_num',$v['goods_num']);
		    						}
    		...
    		$partner_stock = DB::name('partner_stock')->where(['partner'=>$partner_id])->select();
    		foreach ($partner_stock as $key => $value) {
    			foreach ($machine_conf as $k => $v) {
    				if ($value['goods_id'] == $v['goods_id']) {
    					//已有此商品 增加
    					DB::name('partner_stock')->where(['partner_id'=>$partner_id])->setInc('stock_num', $v['goods_num']);
    				}else{
    					//不存在 新添
    					$stock = array(
    						'partner_id'=>$partner_id,
              ...
              foreach ($machine_stock as $k => $v) {
              	foreach($diff as $key => $value){
                     ...
                     if ($v['goods_id']==$value) {
                            ...
                            var_dump($default_key);
                            $del_goods_key = $default_key-($count/2);var_dump($del_goods_key);die;
                            $del_goods_value = $default_arr[$del_goods_key];
