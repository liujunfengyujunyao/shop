...
class Partner extends Model {
	...
	public function max_stock($partner_id, $goods_id) {
		...
		$ids = DB::name('machine')->where(array('partner_id'=>$partner_id))->getField('machine_id', true);
		$max = 0;
		foreach ($ids as $key => $value) {
			$num = Db::name('machine_stock')->where(array('machine_id'=>$value, 'goods_id'=>$goods_id))->getField('stock_num');
			if (empty($num)) {
