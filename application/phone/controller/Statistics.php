...
//			$machine_arr = DB::name('machine')->where(['client_id'=>$manager_info['admin_id']])->getField('machine_id',true);
    ...
    public function list_index(){
        ...
        if(!$belong_id){
            ...
            $data = DB::name('client_day_stat_2019_1')->where("client_id = $client_id")->order("stat_period desc")->select();
        }else{

            $data = DB::name('client_day_stat_2019_1')->where("client_id = $belong_id")->order("stat_period desc")->select();
        }
    ...
    public function detail(){
        $client_id = $_SESSION['think']['client_id'];
        $date = I('get.stat_period');
        $end = $date+60*60*24-1;
        $data = DB::name('client_day_stat_2019_1')->where("client_id = $client_id && stat_period = $date")->find();
        // halt($data);
