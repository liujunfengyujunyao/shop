...
class Index extends Controller {
	...
	public function rooms_status($params){
		...
		foreach ($rooms as $key => $value) {
			...
			DB::name('client_machine_conf')->where(['machine_id'=>$machine['machine_id'],'location'=>$value['roomid']])->save(['status'=>$value['status']]);
			// DB::name('client_machine_stock')->where(['machine_id'=>$machine['machine_id'],'location'=>$value['roomid']])->save(['status'=>$value['status'],'stock_num'=>$value['stocks']]);
		}
