...
//                    halt($value);
    ...
    public function output_error($params){
        ...
        $machine = DB::name('machine')->where(['sn'=>$params['machinesn']])->find();
        $total = DB::name('client_machine_conf')->where(['machine_id'=>$machine['machine_id'],'location'=>$error['roomid']])->getField('goods_price');
        if(!$total){
            $msg = array(
        ...
        if($error['failednumber'] != 0){
            DB::name('refund')->add(['machine_id'=>$machine['machine_id'],'businesstype'=>$error['businesstype'],'roomid'=>$error['roomid'],'logid'=>$error['logid'],'failednumber'=>$error['failednumber'],'time'=>time(),'total'=>$total]);
        }
