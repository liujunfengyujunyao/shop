<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:49:"./application/admin/view2/partner\add_config.html";i:1533876247;s:44:"./application/admin/view2/public\layout.html";i:1533876247;}*/ ?>
<!doctype html>
<html>
<head>
						...
						<tr>
							...
							<td align="left" class="">
								<div style="text-align: center; width: 620px;">
									<select onchange="getId(this)" <?php if($act == 'edit'): ?>disabled<?php endif; ?>>
										<option value="">请选择</option>
										<?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?>
										<option value="<?php echo $v['goods_id']; ?>" <?php if(\think\Request::instance()->param('id') == $v['goods_id']): ?>selected<?php endif; ?>><?php echo $v['goods_name']; ?></option>
							...
							<td align="left" class="">
								<div style="text-align: center; width: 155px;">
									<input type="text" class="number" value="<?php echo (isset($stock) && ($stock !== '')?$stock:1); ?>" onkeyup='this.value=this.value.replace(/\D/gi,"")' name='stock'/>
								</div>
							</td>
...
function checkForm(){
	...
	if (arr.length == 0) {
		...
		var id = "<?php echo \think\Request::instance()->param('id'); ?>";
		$.ajax({
			type: "POST",
			...
			success: function (data) {
				if (data.status == 1) {
					layer.msg(data.msg, {icon: 1, time: 2000}, function() {
						...
						parent.location.href = "<?php echo U('Admin/Partner/stockConfig'); ?>";
					});
				} else {
