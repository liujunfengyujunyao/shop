<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:50:"./template/pc/rainbow/partner\act_store_apply.html";i:1533876251;}*/ ?>
<!Doctype html>
<html>
<head>
<meta charset="utf-8">
<title>配货-<?php echo $tpshop_config['shop_info_store_title']; ?></title>
					...
					<tr>
						...
						<td>
							<select onchange="getId(this)">
								...
								<option value="<?php echo $v['goods_id']; ?>" max="<?php echo $v['max_num']; ?>"><?php echo $v['goods_name']; ?></option>
...
function addLine(){
	...
	$(".apply .box-ct table tbody tr:last .number").css("border", '1px solid #dfdfdf');
}
...
function checkForm(){
	...
	if (arr.length == 0) {
		...
		$.ajax({
			type: "POST",
			url: "/index.php?m=Home&c=Partner&a=act_store_apply&id="+id,
			...
			success: function (data) {
				if (data.status == 1) {
					layer.msg(data.msg, {icon: 1, time: 2000}, function() {
						...
						parent.location.href = "<?php echo U('Partner/delivery'); ?>";
					});
				} else {
